...
static struct usb_device_id rtw_usb_id_tbl[] = {
	...
	/*=== Realtek demoboard ===*/
#ifdef CONFIG_RTL8852A
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0x8832, 0xff, 0xff, 0xff), driver_info = RTL8852B},
#endif
...
static int rtw_dev_probe(struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
	...
	RTW_INFO("[DBG][probe] vid=0x%04x pid=0x%04x driver_info=0x%lx interface_number=%u\n",
	pdid->idVendor, pdid->idProduct, (unsigned long)pdid->driver_info,
	pusb_intf->cur_altsetting ? pusb_intf->cur_altsetting->desc.bInterfaceNumber : 0);
	RTW_INFO("[DBG][probe] usb speed=%d (0=unk,1=low,2=full,3=high,4=wireless,5=supper,6=super+)\n",
	interface_to_usbdev(pusb_intf)->speed);
	...
	if (dvobj == NULL) {
		...
		RTW_ERR("[DBG][probe] fail at usb_devobj_init\n");
		goto exit;
	}

	if (devobj_trx_resource_init(dvobj) == _FAIL)
	{
		RTW_ERR("[DBG][probe] fail at devobj_trx_resource_init\n");
		goto free_dvobj;
	}
	...
	if (rtw_hw_init(dvobj) == _FAIL) {
		...
		RTW_ERR("[DBG][probe] fail at rtw_hw_init\n");
		goto free_trx_reso;
	}
	...
	if (padapter == NULL) {
		...
		RTW_ERR("[DBG][probe] fail at rtw_usb_primary_adapter_init\n");
		goto free_hw;
	}

	if (usb_reprobe_switch_usb_mode(padapter) == _TRUE) {
		RTW_ERR("usb_reprobe_switch_usb_mode Failed!\n");
		RTW_ERR("[DBG][probe] fail at usb_reprobe_switch_usb_mode\n");
		goto free_if_prim;
	}
	...
	{
		RTW_ERR("[DBG][probe] fail at rtw_drv_add_vir_ifaces\n");
		goto free_if_vir;
	}
#endif
	...
	{
		RTW_ERR("[DBG][probe] fail at rtw_adapter_link_init\n");
	...
	if (devobj_data_init(dvobj) == _FAIL) {
		...
		RTW_ERR("[DBG][probe] fail at devobj_data_init\n");
	...
	if (rtw_os_ndevs_init(dvobj) != _SUCCESS) {
		...
		RTW_ERR("[DBG][probe] fail at rtw_os_ndevs_init\n");
		goto free_devobj_data;
	}
...
#endif
	RTW_INFO("-%s success\n", __func__);
	RTW_INFO("[DBG][probe] success, ic_id=0x%x iface_nums=%u hw_init_completed=%d\n");
	return 0; /*_SUCCESS*/

free_devobj_data:
	devobj_data_deinit(dvobj);
	RTW_ERR("[DBG][probe] cleanup at free_devobj_data label\n");

free_adapter_link:
	rtw_adapter_link_deinit(dvobj);
	RTW_ERR("[DBG][probe] cleanup at free_adapter_link label\n");

#ifdef CONFIG_CONCURRENT_MODE
free_if_vir:
	rtw_drv_stop_vir_ifaces(dvobj);
	rtw_drv_free_vir_ifaces(dvobj);
	RTW_ERR("[DBG][probe] cleanup at free_if_vir label\n");
#endif

free_if_prim:
	if (padapter)
		rtw_usb_primary_adapter_deinit(padapter);
	RTW_ERR("[DBG][probe] cleanup at free_if_prim label\n");
free_hw:
	rtw_hw_deinit(dvobj);
	RTW_ERR("[DBG][probe] cleanup at free_hw label\n");

free_trx_reso:
	devobj_trx_resource_deinit(dvobj);
	RTW_ERR("[DBG][probe] cleanup at free_trx_reso label\n");

free_dvobj:
	usb_dvobj_deinit(pusb_intf);
	RTW_ERR("[DBG][probe] cleanup at free_dvobj label\n");
exit:
	return -ENODEV;
}
